	...
	private static final Properties props = PropsLoaderUtil.getProperties("secret/aws_s3.properties");
		...
		public static void main(String[] args) {
			...
			PutObjectRequest por = PutObjectRequest.builder() 
					...
					.build();
			s3.putObject(por, RequestBody.fromFile(new File("C:\\Users\\tj\\semi_gamjas_kiy\\semiproject\\GAMJAS2\\src\\main\\java\\util")));
		}
		...
		private static void uploadInternal (InputStream is, String key, long size, String contentType) {
			PutObjectRequest por = PutObjectRequest.builder() 
					...
					.build();
			try(is) {
				s3.putObject(por,RequestBody.fromInputStream(is, size));
			} catch (IOException e) {
